...
#
#### LOG VERBOSITY
# 0 - NONE--: No track any message
# 1 - ERROR-: Trak only error messages
# 2 - WARN--: Track warning messages
# 3 - INFO--: Track info messages
# 4 - DEBUG-: Track debug messages
#
# EOH
...
function usage_info_env(){
  ...
  echo "  -l | --lab   Laboratory Environment"
...
function create_sym_link(){
  ...
  if [ -L "${tool_local}" ]; then
    sudo unlink "${tool_local}"
  fi
  sudo ln -s "${tool}" "${tool_local}"

  msg "Symbolic link ${tool_local} created!"
}
